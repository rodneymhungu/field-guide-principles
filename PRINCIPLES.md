...
How I build single-page technical guides, and why. These rules came out of the [Agent 365 field guide](https://rodneymhungu.github.io/agent-365-guide/) in September 2026 and now apply to every guide I publish. If a page and this document disagree, this document wins and the page gets fixed.

A field guide is one HTML page about one product or one problem, written for the engineer who has to make it work. It is sourced, opinionated, and honest about what is not ready. It is not a brochure or a summary of the docs.
...
**Opinion is quarantined.** Judgement calls live in named, numbered field notes, visibly separate from the sourced facts. The footer says whose opinions they are. A reader can always tell which sentences the vendor would sign and which ones are mine. Field notes describe things that happened. Nobody in them is invented, and neither is anything they said. When a note disagrees with the vendor or with common practice, it names the real tension instead of scolding.
...
- Product names in full and consistent. Distinct products stay distinct: the guide never implies one lives inside another, and says how they connect. Status words (preview, GA, licence-dependent) are precision and are never cut for style.
...
The page should look like a working document someone carries, not a marketing site. Quiet confidence and one memorable detail. Nothing should make a sceptical reader wonder whether the substance is thin.
...
- **Three typefaces with three jobs.** A serif with optical sizes for display and reading, a sans for interface and body, a mono for labels, captions, sources and anything that is data. A hand-drawn face appears only inside sketches. None of the three is a default. Inter, Roboto and the system stack mark a page as generated.
...
- One page per topic, one data file. A guide starts as one HTML file. When a chapter deserves to be found on its own, it becomes a second page that loads the same data file, shared styles and shared behaviour, and the build fails if the chapter differs between pages. No build step, no framework, no dependency beyond fonts. Anyone can read the source and see how it works.
...
- Screenshots show demo-tenant data cleared for publication. Nothing in them identifies a customer.
...
A guide starts going stale the week it ships, so keeping it current is part of building it.

- The review date lives in the data block and renders in the footer. A reader can see how old the page is without asking.
- Every cited source page is re-fetched on a schedule. When one changes, the change opens a pull request with the smallest edit that keeps the guide true. Nothing merges itself.
- A status flip is merged only after the cited page has been read. A wrong status is worse than a stale one.
- Feedback arrives as issues on the repository. The digest of visits and mentions is public, so it names referrers by hostname only. No path, and nothing that identifies a reader.
- The repository stores no credential the guide does not need. If a feature needs a stored credential, the feature is dropped.
